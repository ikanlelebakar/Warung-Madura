...
Element RenderCheckout(const std::vector<std::string>& cartHeaders, const std::string& paymentMethod) {
    // Updated to show payment method selection
    std::string methodDisplay = paymentMethod.empty() ? "CASH" : paymentMethod;
    ...
    return StyledBox(vbox({
        ...
        separator(),
        hbox({text("Metode Pembayaran: ") | bold, text(methodDisplay) | bold | color(Colors::accent())}) | center,
        text("[1] CASH  |  [2] CASHLESS") | center | dim,
        separator(),
        text("Tekan [Y] Bayar | [N] Batal") | center | color(Colors::accent())
    }), "CHECKOUT") | center;
}

// Legacy function for backward compatibility - calls new function with default "CASH"
Element RenderCheckout(const std::vector<std::string>& cartHeaders) {
    return RenderCheckout(cartHeaders, "CASH");
}
...
Element RenderStockAdd(Component& namaInput, Component& jumlahInput, Component& hargaInput, 
                       Component& expDayInput, Component& expMonthInput, Component& expYearInput,
    ...
    return hbox({
        StyledBox(vbox({
            ...
            separator(),
            text("Tanggal Expired (0 = tidak ada):") | dim,
            hbox({text("Hari   : ") | size(WIDTH, EQUAL, 9), expDayInput->Render() | flex | borderRounded}),
            hbox({text("Bulan  : ") | size(WIDTH, EQUAL, 9), expMonthInput->Render() | flex | borderRounded}),
            hbox({text("Tahun  : ") | size(WIDTH, EQUAL, 9), expYearInput->Render() | flex | borderRounded}),
            separator(), 
            text("Harga jual dihitung otomatis") | center | dim,
            text("[Enter] Simpan | [Esc] Batal") | center | dim
...
Element RenderStockEdit(Component& codeInput, Component& namaInput, Component& jumlahInput, Component& hargaInput,
                        Component& expDayInput, Component& expMonthInput, Component& expYearInput,
                        const std::vector<std::string>& stockHeaders) {
    return hbox({
        StyledBox(vbox({
            ...
            separator(), 
            text("Harga jual dihitung otomatis") | center | dim,
            text("[Enter] Update | [Esc] Batal") | center | dim
